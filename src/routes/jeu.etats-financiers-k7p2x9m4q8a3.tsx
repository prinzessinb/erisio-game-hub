...
const T: Record<Lang, { panels: string[][]; labels: Record<string, string[]>; subtitle: string; joinText: string; teamPlaceholder: string; join: string; clear: string; submit: string; team: string; consigne: string; perfect: string; result: string; links: (c: number, t: number) => string; }> = {
  fr: {
    ...
    join: 'Rejoindre', clear: 'Effacer', submit: 'Valider', team: 'Équipe',
  ...
  en: {
    ...
    join: 'Join', clear: 'Clear', submit: 'Submit', team: 'Team',
...
function EtatsFinanciers() {
  ...
  async function join() {
    ...
    if (room) { setStartedAt(new Date(room.started_at).getTime()); if (room.validated) { setValidated(true); setScore(room.score || 0); setCorrect(room.correct || 0); } }
  ...
  useEffect(() => {
    ...
    const tick = () => { const rem = Math.max(0, DURATION - Math.floor((Date.now() - startedAt) / 1000)); setRemaining(rem); if (rem <= 0 && !validatedRef.current) validate(); };
    tick();
    ...
    <div style={S.page}>
      <header style={S.header}>
        ...
        <div style={S.hdrRight}>
          ...
          <div style={{ ...S.chrono, ...(low ? S.chronoLow : null) }}>⏱ {fmt(remaining)}</div>
        </div>
      </header>
      <main style={S.main}>
        ...
        <div style={S.toolbar}>
          ...
          <button style={S.btnPrimary} onClick={validate} disabled={validated}>{t.submit}</button>
        </div>
        <div style={S.stage}>
...
const S: Record<string, CSSProperties> = {
  ...
  card: { background: '#fff', borderRadius: 16, maxWidth: 420, width: '100%', padding: 28, textAlign: 'center', boxShadow: '0 18px 50px rgba(0,0,0,.3)' },
